...
void AnalisiMCPar2(Int_t seed = 2)
{

 gStyle->SetOptTitle(0);

TChain *ch = new TChain("tree","");
ch->Add("ToyMC_Par2.root/tree");  
TTree *tree = (TTree*) ch;
classtoy t(tree); 
...
RooRealVar widthYi("widthYi","widthYi",1.0,0.0,2.0);
...
canv_Yipull->Print(Form("Pull_YiBs_ToyMC_%1i.pdf",seed));
...
canv_Yibpull->Print(Form("Pull_Yib_ToyMC_%1i.pdf",seed)); 

RooRealVar meanMu("meanMu","meanMu",0.0,MminP,MmaxP);
RooRealVar widthMu("widthMu","widthMu",1.0,0.0,2.0);
...
canv_Mupull->Print(Form("Pull_Mu_ToyMC_%1i.pdf",seed)); 
